  ...
  return async (dispatch, getState) => {
    try {
      ...
      const response = await axios.post(
        `https://vm9qie5ock.execute-api.ap-southeast-1.amazonaws.com/prod/api/auth/reset/${token}`,
        user
      );
...
export const resetAccountPassword = () => {
  return async (dispatch, getState) => {
    try {
      ...
      const response = await axios.post(
        `https://vm9qie5ock.execute-api.ap-southeast-1.amazonaws.com/prod/api/auth/reset`,
        user
      );
      const successfulOptions = {
        title: `${response.data.message}`,
